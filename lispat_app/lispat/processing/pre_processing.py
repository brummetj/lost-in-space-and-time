...
class Preproccessing:
    ...
    def get_docs_dir(self, submission):
        ...
        if submission['--compare']:
            ...
            path_docx =  "/usr/local/var/lispat/pdf_data/"
            path_csv = "/usr/local/var/lispat/csv_data/" # not used but we need to use all of the data.
            path_array = [path, path_docx] # Only docx and pdf are used right now.
    ...
    def get_doc(self):
        ...
        if self.word:
            text = open(self.word[0][0][0], 'rt')
            self.txt_data = text.read()

        if self.pdf:
            text = open(self.pdf[0][0][0], 'rt')
            self.txt_data = text.read()
    ...
    def filter_nlp(self):
        ...
        try:
            if self.pdf_path:
                logger.getLogger().debug("Running pre-processing on directory: " + self.pdf_path)

            if self.pdf:
                logger.getLogger().debug("Running pre-processing on file: " + self.pdf[0][0][0])

            if self.word:
                logger.getLogger().debug("Running pre-processing on file: " + self.word[0][0][0])
    ...
    def word_count(self):
        ...
        try:
            ...
            logger.getLogger().error(bcolors.FAIL + "Pylot error, please check stack trace" + bcolors.ENDC)
            sys.exit(1)
